...
Note that we can 
- either specify an ID value, in which case it becomes the key - which is a unique string identifying the credential,
required to create the JWT token
- or get an autogenerated value by leaving that parameter out. 

In either case, the secrets-config utility returns the key value. It's also possible to later get this key value using

```
curl -X GET http://localhost:8001/consumers/user05/jwt | jq
```

The secrets-config command is doing the equivalent of:
